...
It is the connection-time counterpart to
[`configauth`](../configauth/README.md). `configauth` handles **transport-level**
authentication for outgoing HTTP/gRPC requests (a `RoundTripper` or
`PerRPCCredentials` per request) and carries no notion of expiry. A database
driver needs something different: a credential at connection-open time that may
expire (an AWS RDS IAM token lives ~15 minutes) or change out of band (a rotated
